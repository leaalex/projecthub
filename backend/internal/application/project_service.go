...
// ListNoteSections возвращает секции заметок проекта.
func (s *ProjectService) ListNoteSections(ctx context.Context, projectID uint) ([]*project.NoteSection, error) {
	p, err := s.Projects.FindByID(ctx, project.ID(projectID))
	if err != nil {
		return nil, err
	}
	out := make([]*project.NoteSection, 0, len(p.NoteSections()))
	for _, sec := range p.NoteSections() {
		out = append(out, sec)
	}
	return out, nil
}

// AddNoteSection добавляет секцию заметок.
func (s *ProjectService) AddNoteSection(ctx context.Context, projectID uint, name string) (*project.NoteSection, error) {
	p, err := s.Projects.FindByID(ctx, project.ID(projectID))
	if err != nil {
		return nil, err
	}
	sec, err := p.AddNoteSection(name, s.now())
	if err != nil {
		return nil, err
	}
	if err := s.Projects.Save(ctx, p); err != nil {
...
func (s *ProjectService) RenameNoteSection(ctx context.Context, projectID, sectionID uint, name string) (*project.NoteSection, error) {
	p, err := s.Projects.FindByID(ctx, project.ID(projectID))
	if err != nil {
		return nil, err
	}
	sid := project.NoteSectionID(sectionID)
	if err := p.RenameNoteSection(sid, name, s.now()); err != nil {
		return nil, err
	}
	if err := s.Projects.Save(ctx, p); err != nil {
		return nil, err
	...
	return p2.NoteSectionByID(sid), nil
}

// DeleteNoteSection удаляет секцию заметок.
func (s *ProjectService) DeleteNoteSection(ctx context.Context, projectID, sectionID uint) error {
	p, err := s.Projects.FindByID(ctx, project.ID(projectID))
	if err != nil {
		return err
	}
	if err := p.RemoveNoteSection(project.NoteSectionID(sectionID), s.now()); err != nil {
		return err
	}
	return s.Projects.Save(ctx, p)
}

// ReorderNoteSections задаёт порядок секций заметок.
func (s *ProjectService) ReorderNoteSections(ctx context.Context, projectID uint, sectionIDs []uint) error {
	p, err := s.Projects.FindByID(ctx, project.ID(projectID))
	if err != nil {
		return err
	}
	order := make([]project.NoteSectionID, len(sectionIDs))
	for i, id := range sectionIDs {
		order[i] = project.NoteSectionID(id)
	}
	if err := p.ReorderNoteSections(order, s.now()); err != nil {
		return err
	}
	return s.Projects.Save(ctx, p)
}
