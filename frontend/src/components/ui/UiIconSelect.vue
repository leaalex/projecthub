...
export type UiIconSelectOption<T extends string | number = string | number> = {
  value: T
  label: string
  /** Иконка слева у опции и (по умолчанию) в триггере при одиночном выборе. */
  icon?: Component | FunctionalComponent
  disabled?: boolean
}

export type UiIconSelectModelValue = string | number | (string | number)[]
...
const props = withDefaults(
  defineProps<{
     ...
     */
    modelValue: UiIconSelectModelValue
    options: UiIconSelectOption<string | number>[]
    id?: string
    label?: string
    placeholder?: string
    disabled?: boolean
    ...
    multiple?: boolean
    /** Иконка в триггере (для multiple — основной способ задать иконку кнопки). */
    triggerIcon?: Component | FunctionalComponent
    /** Accessible name для combobox, если нет видимого `label`. */
    ariaLabel?: string
    /** Подсказка на триггере; в multiple дополняет `displayTitle`. */
    triggerTitle?: string
  }>(),
  {
    placeholder: 'Select…',
...
const emit = defineEmits<{
  'update:modelValue': [value: UiIconSelectModelValue]
...
const uid = useId()
const baseId = computed(() => props.id ?? `ui-icon-select-${uid}`)
...
type FilterRow = {
  opt: UiIconSelectOption<string | number>
  sourceIndex: number
}
...
const triggerIconComponent = computed(() => {
  ...
  if (!props.multiple) return selectedOption.value?.icon
  return undefined
})

const showTriggerIcon = computed(
  () => !!slots['trigger-icon'] || triggerIconComponent.value != null,
)
...
<template>
  ...
  >
    ...
    <button
      ...
      :disabled="disabled"
      :title="triggerTitle ?? displayTitle"
      :class="[
        'flex w-full items-center gap-2 rounded-md border border-border bg-surface text-left transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50',
        triggerClass,
    ...
    >
      <span
        ...
        aria-hidden="true"
      >
        <slot name="trigger-icon" :selected="selectedOption">
          <component :is="triggerIconComponent" class="h-4 w-4" />
        </slot>
      </span>
            ...
            >
              ...
              >
                <slot
                  name="option-icon"
                  :option="row.opt"
                  :selected="isSelectedValue(row.opt.value)"
                  :active="fi === activeIndex"
                >
                  <component
                    v-if="row.opt.icon"
                    :is="row.opt.icon"
              ...
              <span class="min-w-0 flex-1 truncate">{{ row.opt.label }}</span>
              <span
