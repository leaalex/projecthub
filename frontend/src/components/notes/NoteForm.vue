...
const props = withDefaults(
  ...
  { initial: null, defaultSectionId: null, loading: false },
)

const emit = defineEmits<{
  submit: [payload: { title: string; body: string; section_id: number | null }]
  cancel: []
}>()
...
watch(
  () => [props.initial, props.defaultSectionId] as const,
  ([n, defSid]) => {
    if (n) {
      title.value = n.title
      body.value = n.body ?? ''
